...
try{
  ...
  if(manifest.schemaVersion!==2||manifest.scope!=='RUNTIME_ARTIFACTS'||manifest.version!==release.candidateVersion||JSON.stringify(manifest.files.map(v=>v.file))!==JSON.stringify(files))throw new Error('CONTENT_MANIFEST_INVALID');
  ...
  process.stdout.write(JSON.stringify({ok:true,version:release.candidateVersion,status:release.status,artifactFiles:files.length,artifactDigest:digest(manifest.files.map(e=>e.sha256).join(''))})+'\n');
